...
class SRMBRepMasker(Pruner):
	...
	def generate_masks(self, model, is_static=True, verbose=True):
		use_same_mask = False
		...
		for layer in self.layer_configs:
			...
			# print(f"Name of layer is {layer}")
	...
	def construct_mask(tensor, config):
		...
		print(f"rows is {rows}, cols  is {cols} and kernel size is {kernel_size}")
		# Modified stuff
		...
		if config.is_repetitive:
			...
			print(f"Outer Block mat shape is {OBmat.shape}")
			CBmat = np.ones((obh//cbh, obw//cbw), dtype=tensor.dtype)
			print(f"Shape of Central Block matrix is {CBmat.shape}")
	...
	def test():
		...
		config =  SRMBRepMaskerConfig(obh, obw, cbh, cbw, ibh, ibw, osp, opat, isp, ipat, is_repetitive, collapse_tensor, cross_prob, is_symmetric)
